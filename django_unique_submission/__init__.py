...
_UNIQUE_TIMEOUT = getattr(settings, 'UNIQUE_TOKEN_TIMEOUT', 18000) #5 hours*3600 = 18000 seconds
...
def unique(issue_method="GET", validate_method='POST'):
    ...
    def _controller(view_func): 
        def wrapper(request, *args, **kwargs):
            if not request.session.has_key(_UNIQUE_TOKEN_ID):
            ...
            if request.method == validate_method:
                ...
                # start checking
                token = request.POST.get(_UNIQUE_TOKEN_KEY) or request.GET.get(_UNIQUE_TOKEN_KEY)
                ...
                available_tokens[utok] = datetime.datetime.now() + datetime.timedelta(seconds=_UNIQUE_TIMEOUT)
